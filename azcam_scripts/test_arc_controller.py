...
def test_arc_controller(cycles=10):
    ...
    if azcam.db.controller.pci_board_installed:
        azcam.log("Testing PCI board...")
        for loop in range(cycles):
            azcam.db.controller.test_datalink(1, 111, 1)
        azcam.log("PCI board OK")

    if azcam.db.controller.timing_board_installed:
        azcam.log("Testing Timing board...")
        for loop in range(cycles):
            azcam.db.controller.test_datalink(2, 222, 1)
        azcam.log("Timing board OK")

    if azcam.db.controller.utility_board_installed:
        azcam.log("Testing Utility board...")
        for loop in range(cycles):
            azcam.db.controller.test_datalink(3, 333, 1)
    ...
    if azcam.db.controller.utility_board_installed:
        ...
        for loop in range(cycles):
            azcam.db.tempcon.get_temperatures()
